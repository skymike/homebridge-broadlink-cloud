...
- [x] End-to-end integration: mock setup flow + real read-only existing-account discovery; npm/package compatibility matrix, typecheck/build and package secret scan; independent review.
- [x] Backed-up production deployment on homebridge.local preserving config/pairing/session. Verify UI endpoints, live discovery, all existing HAP accessories, persistence and one polling window. Do not physically operate appliances without specific test intent.
...
Production verification: 0.3.0 installed with rollback artifacts; CI 35031646328 passed. Actual Homebridge custom UI discovers the hub and three remotes, renders all 13 fan command choices and existing six speed mappings. Both fans' Active/RotationSpeed and all seven AC control reads succeed; room sensors respond. Three cached accessories and existing BroadLink config preserved. No BroadLink startup/refresh errors after a polling window. No physical commands sent.
